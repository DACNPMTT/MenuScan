...
import { AlertCircle, ArrowLeft, Check, Loader2, XCircle } from 'lucide-react'
import { useAuth } from '@/app/providers/AuthProvider'
import { apiRequest, ApiError } from '@/shared/lib/api'
import { useDocumentTitle } from '@/shared/hooks/useDocumentTitle'
import type { MenuItemResult, ScanDetail, ScanResult } from '@/features/menu-scan/types'
    ...
    const poll = async () => {
      try {
        ...
        if (current.status === 'FAILED') {
          if (!cancelled) setError(current.error || 'Quét thất bại. Vui lòng thử lại.')
          return
        }
  ...
  return (
    <div className="mx-auto w-full max-w-[900px] px-[30px] py-[40px] sm:px-[50px]">
      ...
      {error && (
        <div
          role="alert"
          className="flex items-start gap-3 rounded-[12px] border border-destructive/30 bg-destructive/5 px-4 py-3 text-[14px] text-destructive"
        >
...
/** The source endpoint requires a Bearer token, so <img src> can't reach it
 *  directly. Fetch the bytes with auth, then render an object URL. */
function SourcePreview({
  ...
  useEffect(() => {
    if (!accessToken || !isImage) return
    let url: string | null = null
    let active = true
    fetch(source.preview_url, {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {items.length === 0 ? (
        ...
        <ul className="flex flex-col divide-y divide-hairline overflow-hidden rounded-[12px] border border-hairline bg-canvas">
          {items.map((item) => (
            <li
              key={item.id}
