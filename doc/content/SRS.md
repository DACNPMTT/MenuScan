...
- Tìm kiếm và lọc món **trong một menu** theo tên/giá/nhóm.
- Chọn món, thêm phí/thuế/giảm giá, chia bill theo số người; xem lịch sử bill.

**Trợ lý chọn món cá nhân hóa (đã triển khai):**

- Hồ sơ ăn uống (`food_profiles`): dị ứng, món tránh, khẩu vị ưa thích, quy tắc
  ăn kiêng — mỗi mục có `preference_type`, `intensity` và `importance`.
- Lượt LLM thứ hai (`POST /menus/{id}/enrich`) suy luận nguyên liệu, food tag và
  mức vị (cay/ngọt/mặn/chua/béo/dầu mỡ) cho từng món.
- Verdict gợi ý theo hồ sơ (`RECOMMENDED` / `OK` / `CAUTION` / `AVOID`) kèm lý do
  phù hợp và lý do nên tránh; cảnh báo dị ứng được giữ nguyên.
- Chat hỏi trợ lý về menu (`POST /advisor/chat`), grounding trên món của chính
  menu đó cộng hồ sơ ăn uống.
- Luồng nhóm (dining session): host tạo phiên, chia link mời; thành viên khai
  khẩu vị **không cần đăng nhập**; verdict được chấm trên tập preference của cả
  bàn.
...
| FR-17 | User tạo/sửa/xóa hồ sơ ăn uống với preference có `intensity` và `importance`. | Must |
| FR-18 | Hệ thống chạy lượt LLM thứ hai để sinh food tag, mức vị và cảnh báo rủi ro cho từng món. | Must |
| FR-19 | Hệ thống chấm verdict gợi ý cho từng món theo hồ sơ, kèm lý do phù hợp và lý do nên tránh. | Must |
| FR-20 | Host tạo phiên ăn nhóm và chia link mời có hạn dùng. | Must |
| FR-21 | Thành viên tham gia phiên ăn và khai khẩu vị **không cần tài khoản**. | Must |
| FR-22 | User hỏi trợ lý về một menu đã scan; câu trả lời grounding trên món của menu đó và hồ sơ ăn uống. | Must |
| FR-23 | Hệ thống giới hạn tần suất gọi AI (scan, chat) theo user hoặc IP. | Must |
