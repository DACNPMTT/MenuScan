...
def parser_results(ground_truth_samples, parser) -> dict[str, dict]:
    ...
    for sample in ground_truth_samples:
        ...
        expected_names = {
            item["name"].lower().strip() for item in sample["items"] if "name" in item
        }
...
class TestParserQualityBaseline:
    ...
    def test_aggregate_price_accuracy_above_threshold(self, parser_results):
        total_price_acc = sum(
            r["metrics"].price_accuracy for r in parser_results.values()
        )
    ...
    def test_single_column_samples_parsed(self, parser_results):
        sc_results = [
            r for r in parser_results.values() if "single_column" in r["sample"]["tags"]
        ]
    ...
    def test_multi_column_samples_parsed(self, parser_results):
        mc_results = [
            r for r in parser_results.values() if "multi_column" in r["sample"]["tags"]
        ]
    ...
    @pytest.mark.xfail(
        reason="RuleBasedMenuParser does not yet extract variant names from variants"
    )
    def test_variant_menu_extracts_base_and_variants(self, parser_results):
        var_results = [
            r for r in parser_results.values() if "variant_menu" in r["sample"]["tags"]
        ]
        assert len(var_results) > 0
        for r in var_results:
            draft = r["draft"]
            has_variants = any(
                item.base_name and item.variant_name for item in draft.items
            )
